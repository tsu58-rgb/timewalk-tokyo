...
import { timelineChallenges } from "@/data/shonen-yamada/challenges";
import { historyEvents } from "@/data/shonen-yamada/events";
import type { HistoryEvent, TimelineChallenge as TimelineChallengeType } from "@/types/shonen-yamada";
...
const difficultyLabels: Record<TimelineChallengeType["difficulty"], string> = {
  easy: "入門",
  normal: "ふつう",
  hard: "むずかしい",
};
...
export default function TimelineChallenge() {
  ...
  const challenge = timelineChallenges[challengeIndex];
  ...
  const isLastChallenge = challengeIndex === timelineChallenges.length - 1;
  ...
  function checkAnswer() {
    if (selectedEventIds.length !== challengeEvents.length) {
      setResult({
        correct: false,
        message: "まだ年表が完成していないよ。すべてのカードを選んでね。",
      });
      return;
    }
    ...
    setResult({
      correct: false,
      message: "惜しい！年代の小さい出来事から順番に見直してみよう。",
    });
  }

  function goToNextChallenge() {
    setChallengeIndex((current) => Math.min(current + 1, timelineChallenges.length - 1));
    resetChallenge();
  }
      ...
      <div className="w-full max-w-md bg-slate-950 border-4 border-white rounded-3xl p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <p className="text-xs text-yellow-300 font-bold">
            クリア {clearedChallengeIds.length}/{timelineChallenges.length}
          </p>
        </div>

        <header className="text-center mb-5">
          <p className="text-xs text-slate-300 mb-1">TimeWalk mini game</p>
        ...
        <section className="bg-slate-800 rounded-2xl p-4 mb-4 border border-slate-700">
          <div className="flex gap-3 items-start">
            ...
            <div>
              <p className="text-sm text-yellow-300 font-bold mb-1">少年山田</p>
              <p className="text-sm text-slate-100 leading-relaxed">
                歴史のできごとを古い順に並べよう！カードをタップすると年表に入るよ。
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-slate-800 rounded-2xl p-4 mb-4">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            <span className="text-xs bg-slate-700 text-slate-200 px-2 py-1 rounded-full font-bold">
              {challengeIndex + 1}/{timelineChallenges.length}問目
            </span>
          </div>
        ...
        <section className="mb-4">
          <h2 className="font-bold mb-2">あなたの年表</h2>
        ...
        <section className="bg-slate-800 rounded-2xl p-4 mb-4">
          ...
          {remainingEvents.length === 0 ? (
            <p className="text-sm text-slate-400">すべてのカードを年表に入れたよ。</p>
          ) : (
            <div className="space-y-2">
        ...
        {showHint && (
          <section className="bg-yellow-100 text-black rounded-2xl p-4 mb-4">
            <h2 className="font-bold mb-2">山田のヒント</h2>
          ...
          >
            ...
            {result.correct && (
              <div className="mt-3 bg-white/15 rounded-xl p-3">
                <p className="text-sm font-bold mb-2">正しい年表</p>
        ...
        <section className="bg-slate-800 rounded-2xl p-4">
          ...
          <div className="space-y-2">
            {timelineChallenges.map((item, index) => (
              <button
                key={item.id}
                type="button"
