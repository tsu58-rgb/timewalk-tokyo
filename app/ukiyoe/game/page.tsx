...
export default function UkiyoeGamePage() {
  ...
  const [current, setCurrent] = useState<UkiyoeSpot | null>(null);
  ...
  function nextQuestion(source = spots) {
    if (source.length < 4) return;
    const answer = shuffle(source)[0];
  ...
  useEffect(() => {
    if (spots.length >= 4 && !current) nextQuestion(spots);
  }, [spots]);
    ...
    <main className="min-h-screen bg-slate-950 p-4 text-white">
      <div className="mx-auto max-w-2xl">
        ...
        <section className="rounded-3xl border border-amber-300/30 bg-slate-900 p-5">
          ...
          <h1 className="mb-4 text-2xl font-bold">この浮世絵、どこ？</h1>

          {error && <p className="rounded-xl bg-red-900 p-4">{error}</p>}
          {!error && !canPlay && <p>ゲームには4件以上のデータが必要です。</p>}

          {current && (
            <>
              <div className="mb-4 rounded-2xl bg-black p-3">
                <img src={current.imageUrl} alt={current.title} className="max-h-[52vh] w-full object-contain" />
              </div>
              <p className="mb-3 text-sm text-slate-300">{current.series}「{current.title}」</p>
              ...
              {selected && (
                <section className="mt-5 rounded-2xl bg-white p-4 text-black">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <Link className="rounded-xl bg-amber-300 px-4 py-2 font-bold text-black" href={`/ukiyoe/${current.id}`}>
                      詳細を見る
                    </Link>
