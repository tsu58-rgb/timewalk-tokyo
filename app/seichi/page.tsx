...
export default async function SeichiPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#fff7df] px-4 py-8 text-[#171717]">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {works.map((work, index) => (
            <Link
              ...
              className={`group relative border-4 border-black p-5 shadow-[6px_6px_0_#111] transition-transform hover:-translate-y-1 ${
                index % 4 === 0
                  ? "bg-[#ff6f91]"
                  : index % 4 === 1
                    ? "bg-[#65d9ff]"
            ...
            >
              <span className="absolute right-3 top-3 rounded-full border-3 border-black bg-white px-3 py-1 text-xs font-black">
                VIEW
              </span>
              ...
              <div className="mt-5 inline-flex items-center rounded-full border-3 border-black bg-white px-4 py-2 font-black group-hover:bg-[#ffd83d]">
                聖地を見る →
              </div>
            </Link>
          ))}
